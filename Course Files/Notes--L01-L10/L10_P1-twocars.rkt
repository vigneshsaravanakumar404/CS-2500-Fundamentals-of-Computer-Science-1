...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname L09_P3-twocars) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(define (cs-temp cs)
  (...
   (cond [(twocars? cs) (twocars-temp cs)]
...
(define (draw-cs cs)
  ...
  (cond [(twocars? cs) (draw-twocars cs)]
...
(define (move-cs/v2 cs)
  ...
  (cond [(twocars? cs) (if (collided? cs)
                           (tc->wreck cs)
                           (move-twocars cs))]
        [(wreck? cs) (move-wreck cs)]))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; MODIFYING RETURN VALUE OF BIG-BANG
; Exercise:
; Modify what the collision program returns:
; just return the x-position of the first car, or the wreck.
; Hint: use a helper function as this is a separate task.
;<DO NOW><SCROLL-RM> Helper: get-final-position : CS->Number
...
(define (get-final-position cs)
   (cond [(twocars? cs) (twocars-x1 cs)]
         [(wreck? cs) (wreck-x cs)]))


; ... and adapt the main function:

; collision : CS -> Number
; Simulate a car crash, returning the final position
; of the first car or the wreck
 
(define (collision-final-pos initial-cs)
  (get-final-position
   (big-bang initial-cs
     [to-draw draw-cs]
     [on-tick move-cs/v2])))
...
; Let's try running it again
#;(collision-final-pos (make-twocars 1 1 500 2))
