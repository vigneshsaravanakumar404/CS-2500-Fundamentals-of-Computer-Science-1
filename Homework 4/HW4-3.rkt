...
; - Faculty
; - Staff
; Interpretation: a member of the Northeastern community

(define NEUPEEP-1
  (make-student NAME-1 DATE-1 40000))
(define NEUPEEP-2
  (make-faculty NAME-2 DATE-2 5 100000))
(define NEUPEEP-3
...
(define (end-date neupeep)
  (cond
    [(student? neupeep) (date->string (end-of-enrollment neupeep))]
    [(faculty? neupeep) (date->string (end-of-contract neupeep))]
    [(staff? neupeep) "<UNKNOWN>"]))

; annual-budget-cost : NEUPeep -> RealNumber
...
(define (annual-budget-cost neupeep)
  (cond
    [(student? neupeep) (- (annual-tuition neupeep))]
